...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_18.md → Pages/cains_jawbone_page_98.md
  - **Why:** weak cross-strand echo: the bigram `course of` appears only on these two page bodies (p18 “for a course of soup…” ↔ p98 “course of action”), and both pages share a “meeting with the man” frame plus an identification anxiety (p18 “afraid he will look just like every one else” ↔ p98 “I wished to be able to describe him”). Treat as a re-stitch lead only unless a second, more specific continuity marker emerges.
  - **Falsifier:** a third page is found using the bigram `course of` in its page body (making the overlap non-unique), and/or boundary re-read shows the scenes are incompatible (p18 manuscript/rupee editorial discussion vs p98 physicians/medical-advice + money handover), and/or Pages/cains_jawbone_page_98.md remains locked to a different predecessor by a stronger phrase-level continuity (currently `p76 → p98`).
  - **Next falsification check:** search for a second independent anchor (named participant, quoted source, place, or a longer shared phrase) that would justify immediate adjacency; otherwise keep this only as a “same-topic” lead.
  - **Phase 6 test (2026-01-02):** body-text scan shows `course of` occurs only on Pages/cains_jawbone_page_18.md and Pages/cains_jawbone_page_98.md, but `money` is **not** unique (also appears on Pages/cains_jawbone_page_2.md as “money’s worth”). No shared corpus-unique 3‑grams (or longer) were found.
  - **Phase 6 test (2026-01-02):** re-read Pages/cains_jawbone_page_18.md ending and Pages/cains_jawbone_page_98.md opening; the register/scene jump (manuscript/rupee editorial discussion ↔ physicians/medical-advice + money handover) reads abrupt with no concrete continuity marker beyond the generic `course of`.
  - **Outcome:** `FAIL` (not credible as immediate predecessor; keep only as a low-weight thematic overlap note).
