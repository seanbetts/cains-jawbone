...
### Proposed order (all 100 pages; v2.26)
...
- **Block I (`N17`, confidence: `CERTAIN` internal):** Pages/cains_jawbone_page_41.md → Pages/cains_jawbone_page_11.md
- **Block K (`N07`, confidence: `LIKELY` internal):** Pages/cains_jawbone_page_97.md → Pages/cains_jawbone_page_93.md
- **Block W (`N06`, confidence: `LIKELY` internal):** Pages/cains_jawbone_page_66.md → Pages/cains_jawbone_page_67.md → Pages/cains_jawbone_page_73.md → Pages/cains_jawbone_page_74.md → Pages/cains_jawbone_page_83.md
...
- **Block M3 (bridge, confidence: `MAYBE`):** Pages/cains_jawbone_page_75.md
- **Block O (`N32`, confidence: `LIKELY` internal):** Pages/cains_jawbone_page_62.md → Pages/cains_jawbone_page_65.md → Pages/cains_jawbone_page_90.md → Pages/cains_jawbone_page_89.md
- **Block S (`N02` dog POV (Aug run), confidence: `LIKELY` internal):** Pages/cains_jawbone_page_22.md → Pages/cains_jawbone_page_79.md → Pages/cains_jawbone_page_48.md → Pages/cains_jawbone_page_88.md → Pages/cains_jawbone_page_39.md → Pages/cains_jawbone_page_10.md → Pages/cains_jawbone_page_99.md → Pages/cains_jawbone_page_8.md → Pages/cains_jawbone_page_19.md → Pages/cains_jawbone_page_57.md → Pages/cains_jawbone_page_64.md → Pages/cains_jawbone_page_61.md → Pages/cains_jawbone_page_46.md
...
### Phase 6 boundary tests for v2.26 (block boundaries)

- **Phase 6 scan (2026-01-04):** recomputed df==2 token/ngram overlaps for v2.26 *block-boundary* adjacencies (page-body only; Notes ignored). v2.26 keeps the v2.25 Block F move (`Pages/cains_jawbone_page_53.md → Pages/cains_jawbone_page_4.md` via df==2 `purpose`, and `Pages/cains_jawbone_page_5.md → Pages/cains_jawbone_page_81.md` via df==2 trigram `was dead what`) and additionally moves Block L to sit immediately after Page 25, so the new boundaries `Pages/cains_jawbone_page_25.md → Pages/cains_jawbone_page_56.md` and `Pages/cains_jawbone_page_71.md → Pages/cains_jawbone_page_33.md` share weak df==2 trigram anchors (`to me that`, `it seemed to`). The df==2 scan flags the following block-boundary adjacencies with **zero** shared df==2 token anchors and **zero** shared df==2 n‑gram anchors (n=2–6): `Pages/cains_jawbone_page_31.md → Pages/cains_jawbone_page_7.md`, `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md`, `Pages/cains_jawbone_page_93.md → Pages/cains_jawbone_page_66.md`, and `Pages/cains_jawbone_page_46.md → Pages/cains_jawbone_page_68.md`.

#### Phase 6 boundary ledger (v2.26; “zero-anchor” block boundaries)

- `Pages/cains_jawbone_page_31.md → Pages/cains_jawbone_page_7.md`: re-read says no continuity; the best replacement lead `p31 → p21` has a phrase-level continuation plus a plausible motion/route cue (speeding/warnings → “turned sharply” into the Henry/body scene), but `p6 → p21` remains the stronger default predecessor inside `N12`. Focused overlap scan (tokens df≤8; n‑grams df≤3) finds **zero** shared low-df anchors. **Status:** placeholder. **Next:** only adopt `p31 → p21` via a minimal block-level restitch that reroutes `p6` (rather than breaking the now-tested `p6 → p21` adjacency), or find a different successor for Page 31 that keeps `p13 → p31` intact.
- `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md`: current v2.26 block boundary (introduced in v2.24 by relocating `Block I2` + `Block J` to follow Page 20); re-read shows no continuity (dog-register ↔ Bunny/Perry “hammer and tongs” pills). **Status:** placeholder. **Next:** find a dog-register successor for Page 11 (or rehome `Block I`) so we don’t just move the zero-anchor boundary.
- `Pages/cains_jawbone_page_93.md → Pages/cains_jawbone_page_66.md`: current v2.26 block boundary (introduced in v2.26 by relocating Block L after Page 25); re-read shows no continuity (Bunny/Perry proof-weighing ↔ school-colours memorial + “Head” nostalgia). **Status:** placeholder. **Next:** either find a stronger successor for Page 93 inside `N07` that avoids feeding directly into `N06`, or test a stronger predecessor for Page 66 (best current lexical-only leads: `p31` via df==2 `lose`, `p33` via df==2 `sorrow`) and see whether that also resolves the `p31 → p7` zero boundary without breaking sentence-locked `N06`.
...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_51.md → Pages/cains_jawbone_page_75.md
  - **Why:** v2.23 bridge insertion to eliminate the unsupported boundary into `N32`: the page-body token `smoke` appears only on Pages/cains_jawbone_page_51.md (“purple smoke…”) and Pages/cains_jawbone_page_75.md (“…when I smoke…”), giving a minimal positive anchor.
  ...
  - **Outcome:** `UNCLEAR` (single-token bridge; used by v2.23 but still provisional).

- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_75.md → Pages/cains_jawbone_page_62.md
  - **Why:** v2.23 bridge insertion to eliminate the unsupported boundary into `N32`: the page-body token `exulted` appears only on Pages/cains_jawbone_page_75.md (“…I exulted.”) and Pages/cains_jawbone_page_62.md (“…cat that horribly exulted…”), giving a minimal positive anchor.
...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_51.md → Pages/cains_jawbone_page_62.md
  ...
  - **Outcome:** `FAIL` (not used in v2.23; superseded by `Pages/cains_jawbone_page_51.md → Pages/cains_jawbone_page_75.md → Pages/cains_jawbone_page_62.md`).
