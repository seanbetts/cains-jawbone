...
### Proposed order (all 100 pages; v2.28)
...
### Phase 6 boundary tests for v2.28 (block boundaries)

- **Phase 6 scan (2026-01-04):** recomputed df==2 token/ngram overlaps for v2.28 *block-boundary* adjacencies (page-body only; Notes ignored). v2.28 retains all v2.27 boundary-fix moves (v2.25 Block F move; v2.26 Block L move; v2.27 moving Block O after Page 31 and restoring `Pages/cains_jawbone_page_74.md → Pages/cains_jawbone_page_75.md` inside `N06`) but additionally relocates Block R + Block N to immediately follow Block H, replacing the former zero-anchor `Pages/cains_jawbone_page_46.md → Pages/cains_jawbone_page_68.md` boundary with the df==2 phrase-gated `Pages/cains_jawbone_page_46.md → Pages/cains_jawbone_page_72.md` (`of course i was`). The df==2 scan flags the following v2.28 block-boundary adjacencies with **zero** shared df==2 token anchors and **zero** shared df==2 n‑gram anchors (n=2–6): `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md` and `Pages/cains_jawbone_page_93.md → Pages/cains_jawbone_page_66.md`.

#### Phase 6 boundary ledger (v2.28; “zero-anchor” block boundaries)

- `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md`: current v2.28 block boundary (introduced in v2.24 by relocating `Block I2` + `Block J` to follow Page 20); re-read shows no continuity (dog-register ↔ Bunny/Perry “hammer and tongs” pills). **Status:** placeholder. **Next:** find a dog-register successor for Page 11 (or rehome `Block I`) so we don’t just move the zero-anchor boundary.
- `Pages/cains_jawbone_page_93.md → Pages/cains_jawbone_page_66.md`: current v2.28 block boundary (introduced in v2.26 by relocating Block L after Page 25); re-read shows no continuity (Bunny/Perry proof-weighing ↔ school-colours memorial + “Head” nostalgia). **Status:** placeholder. **Next:** either find a stronger successor for Page 93 inside `N07` that avoids feeding directly into `N06`, or test a stronger predecessor for Page 66 (best current lexical-only leads: `p31` via df==2 `lose`, `p33` via df==2 `sorrow`) and ensure any re-stitch doesn’t reintroduce new zero-anchor boundaries elsewhere.
...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md
  - **Why:** new v2.24 block boundary created by relocating `Block I2` + `Block J` to follow Pages/cains_jawbone_page_20.md; no scene/prop/cast continuity is currently known.
