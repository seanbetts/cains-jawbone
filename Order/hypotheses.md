...
### Proposed order (all 100 pages; v2.23)
...
- `Pages/cains_jawbone_page_20.md → Pages/cains_jawbone_page_4.md`: re-read says no continuity (Mole/brass ↔ church-buttonhole seaside comic). Focused overlap scan (tokens df≤8; n‑grams df≤3) finds **zero** shared low-df anchors. **Status:** placeholder. **Next:** test the already-known two-step bridge `p20 → p28 → p4` (`stone` then `church`) only if it can be done as a *minimal* restitch without breaking the strong `p28 → p15` (“…wondered if…” / “interesting”) lock.
...
- `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_28.md`: re-read says no continuity (dog-register ↔ Pluckley/church-window “forging” scene). Focused overlap scan (tokens df≤8; n‑grams df≤3) finds only generic shared tokens `since` (df=4) and `supposed` (df=8), with **zero** shared low-df n‑gram anchors. **Status:** placeholder. **Next:** prefer rehoming Page 28’s predecessor to the new `Pages/cains_jawbone_page_4.md → Pages/cains_jawbone_page_28.md` church/book/woman bridge (see join ledger), and separately identify a dog-register successor for Page 11 (or rehome Page 11) so we don’t just move the zero-anchor boundary.
- `Pages/cains_jawbone_page_85.md → Pages/cains_jawbone_page_97.md`: re-read says no continuity beyond high-noise `Henry`. **Status:** placeholder. **Next:** keep `p97` predecessor leads (`p35`, `p60`) and `p85` successor leads (`p47`, `p87`) as re-stitch candidates until a second non-lexical anchor emerges.
...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_5.md → Pages/cains_jawbone_page_49.md
  - **Why:** current v2.23 block boundary into the Blake/Henry-introduction block (`Pages/cains_jawbone_page_49.md → Pages/cains_jawbone_page_13.md` quote lock); no scene/prop/cast continuity is currently known.
...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_71.md → Pages/cains_jawbone_page_66.md
  - **Why:** current v2.23 block boundary between the `N26` breakfast/poison run and the `N06` school/rose-documents run; no scene/prop/cast continuity is currently known.
