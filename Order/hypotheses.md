...
### Proposed order (all 100 pages; v2.25)
...
### Phase 6 boundary tests for v2.25 (block boundaries)

- **Phase 6 scan (2026-01-04):** recomputed df==2 token/ngram overlaps for v2.25 *block-boundary* adjacencies (page-body only; Notes ignored). v2.25 moves Block F (`Pages/cains_jawbone_page_4.md → Pages/cains_jawbone_page_58.md → Pages/cains_jawbone_page_5.md`) to sit immediately after Block B, replacing zero-anchor boundaries `Pages/cains_jawbone_page_53.md → Pages/cains_jawbone_page_81.md` and `Pages/cains_jawbone_page_5.md → Pages/cains_jawbone_page_49.md` with anchored boundaries `Pages/cains_jawbone_page_53.md → Pages/cains_jawbone_page_4.md` (df==2 token `purpose`) and `Pages/cains_jawbone_page_5.md → Pages/cains_jawbone_page_81.md` (df==2 trigram `was dead what`), and making `Pages/cains_jawbone_page_85.md → Pages/cains_jawbone_page_49.md` an anchored boundary (df==2 trigram `i was still`). The df==2 scan flags the following block-boundary adjacencies with **zero** shared df==2 token anchors and **zero** shared df==2 n‑gram anchors (n=2–6): `Pages/cains_jawbone_page_25.md → Pages/cains_jawbone_page_33.md`, `Pages/cains_jawbone_page_31.md → Pages/cains_jawbone_page_7.md`, `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md`, `Pages/cains_jawbone_page_93.md → Pages/cains_jawbone_page_56.md`, `Pages/cains_jawbone_page_71.md → Pages/cains_jawbone_page_66.md`, and `Pages/cains_jawbone_page_46.md → Pages/cains_jawbone_page_68.md`.

#### Phase 6 boundary ledger (v2.25; “zero-anchor” block boundaries)

- `Pages/cains_jawbone_page_25.md → Pages/cains_jawbone_page_33.md`: re-read says hard scene break (Didling/time-factor ↔ debts/cigarette-box). Focused overlap scan (tokens df≤8; n‑grams df≤3) finds **zero** shared low-df anchors. **Status:** placeholder. **Next:** find a successor for Page 25 that is *not* Page 33; treat `p25 ↔ p20` (`N34`) as the highest-value re-stitch direction if it can be reconciled without breaking the Page 18/20 `blank` lock (and keep `p33 ↔ p10` / `gourdoulis` as a blocked but strong predecessor lead).
...
- `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md`: current v2.25 block boundary (introduced in v2.24 by relocating `Block I2` + `Block J` to follow Page 20); re-read shows no continuity (dog-register ↔ Bunny/Perry “hammer and tongs” pills). **Status:** placeholder. **Next:** find a dog-register successor for Page 11 (or rehome `Block I`) so we don’t just move the zero-anchor boundary.
- `Pages/cains_jawbone_page_93.md → Pages/cains_jawbone_page_56.md`: re-read says hard register break (Bunny/Perry proof-weighing ↔ Pippa/breakfast-clairvoyance). **Status:** placeholder. **Next:** find a stronger predecessor for Page 56 inside the `N26` run, or a stronger successor for Page 93 inside `N07`.
- `Pages/cains_jawbone_page_71.md → Pages/cains_jawbone_page_66.md`: df==2 scan suggests both pages likely stitch elsewhere (`p71`→`p7` via `odour`, `p66`←`p31` via `lose`). **Status:** placeholder. **Next:** attempt a *single* minimal trial restitch that reduces the zero-anchor count (avoid moving `N06` unless the new predecessor/successor has 2+ independent anchors).
...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_71.md → Pages/cains_jawbone_page_66.md
  - **Why:** current v2.24 block boundary between the `N26` breakfast/poison run and the `N06` school/rose-documents run; no scene/prop/cast continuity is currently known.
