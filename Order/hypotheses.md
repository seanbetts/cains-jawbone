...
- **Cluster:** Dog POV strand (confidence: `CERTAIN`)
  ...
  - **Resolved check:** POV audit found no unequivocal human-only physical situations in the core Dog POV pages; Pages/cains_jawbone_page_8.md remains the weakest fit but is retained as a `MAYBE` extension within `N02`. Its “powerful glass” can be read as a naïve/figurative description (or as a lens used by “he” while the dog observes), and the same page contains dog-friendly cues (“sausage place… furtively I knew how excellent”) plus strong continuity anchors (bow/jossers; Mr. Hall bell-push) tying it into the strand.
...
- **Cluster:** `N08` King (“Kipling” name-jibe) narrator (confidence: `LIKELY`)
  ...
  - **Rationale:** Pages 85 and 94 share a narrator-name clue (schoolboy “Kipling” jibe → King; narrator sees own name in a window-sign) and a recurring Henry-in-scene frame; Page 82 shares the same Henry-at-table setting plus the same cryptic-transform habits, making it a strong voice-level match even without the explicit window-sign anchor.
...
- **Cluster:** `N02` Dog POV (confidence: `CERTAIN`)
  ...
  - **Bridge candidate (unresolved):** Pages/cains_jawbone_page_57.md shares table-setting overlap with the `N25` Westphalian-ham/papers scene; do not place it into the mid‑August spine unless a compatible date anchor emerges.
