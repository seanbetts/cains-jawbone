...
### Proposed order (all 100 pages; v2.27)
...
### Phase 6 boundary tests for v2.27 (block boundaries)

- **Phase 6 scan (2026-01-04):** recomputed df==2 token/ngram overlaps for v2.27 *block-boundary* adjacencies (page-body only; Notes ignored). v2.27 keeps the v2.25 Block F move (`Pages/cains_jawbone_page_53.md → Pages/cains_jawbone_page_4.md` via df==2 `purpose`, and `Pages/cains_jawbone_page_5.md → Pages/cains_jawbone_page_81.md` via df==2 trigram `was dead what`) and the v2.26 Block L move (placing `Pages/cains_jawbone_page_56.md → Pages/cains_jawbone_page_54.md → Pages/cains_jawbone_page_71.md` immediately after `Pages/cains_jawbone_page_25.md`, and using the weak df==2 trigram boundaries `Pages/cains_jawbone_page_25.md → Pages/cains_jawbone_page_56.md` / `Pages/cains_jawbone_page_71.md → Pages/cains_jawbone_page_33.md`). v2.27 also moves Block O to sit immediately after `Pages/cains_jawbone_page_31.md` (df==2 token anchors: `square` for `Pages/cains_jawbone_page_31.md → Pages/cains_jawbone_page_62.md`, `concomitant` for `Pages/cains_jawbone_page_89.md → Pages/cains_jawbone_page_7.md`) and restores `Pages/cains_jawbone_page_74.md → Pages/cains_jawbone_page_75.md` inside `N06`. The df==2 scan flags the following block-boundary adjacencies with **zero** shared df==2 token anchors and **zero** shared df==2 n‑gram anchors (n=2–6): `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md`, `Pages/cains_jawbone_page_93.md → Pages/cains_jawbone_page_66.md`, and `Pages/cains_jawbone_page_46.md → Pages/cains_jawbone_page_68.md`.

#### Phase 6 boundary ledger (v2.27; “zero-anchor” block boundaries)

- `Pages/cains_jawbone_page_11.md → Pages/cains_jawbone_page_97.md`: current v2.27 block boundary (introduced in v2.24 by relocating `Block I2` + `Block J` to follow Page 20); re-read shows no continuity (dog-register ↔ Bunny/Perry “hammer and tongs” pills). **Status:** placeholder. **Next:** find a dog-register successor for Page 11 (or rehome `Block I`) so we don’t just move the zero-anchor boundary.
- `Pages/cains_jawbone_page_93.md → Pages/cains_jawbone_page_66.md`: current v2.27 block boundary (introduced in v2.26 by relocating Block L after Page 25); re-read shows no continuity (Bunny/Perry proof-weighing ↔ school-colours memorial + “Head” nostalgia). **Status:** placeholder. **Next:** either find a stronger successor for Page 93 inside `N07` that avoids feeding directly into `N06`, or test a stronger predecessor for Page 66 (best current lexical-only leads: `p31` via df==2 `lose`, `p33` via df==2 `sorrow`) and ensure any re-stitch doesn’t reintroduce new zero-anchor boundaries elsewhere.
- `Pages/cains_jawbone_page_46.md → Pages/cains_jawbone_page_68.md`: re-read says incompatible scenes/voices (dog “Hal/Jasmine/chop” ↔ Latham-Chapel “old man” meeting). Focused overlap scan (tokens df≤8; n‑grams df≤3) finds **zero** shared low-df anchors. **Status:** placeholder; **high-priority**. **Next:** keep pursuing a replacement predecessor for Page 68 (best current candidates: `p55` via the df==2 `chord` + df==2 bigram `a chord`; `p65` via `tragedy`; `p13` via `whistling` + “sort of thing was”), or a two-step bridge via `p74` (`education` then `distracted`), but only adopt if it doesn’t break sentence-locked `N06`.
...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_55.md → Pages/cains_jawbone_page_37.md
  ...
  - **Outcome:** `PASS` (adopt as the v2.16 successor for Page 55; keep confidence `MAYBE`).
