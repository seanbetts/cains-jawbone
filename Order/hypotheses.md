...
- `Pages/cains_jawbone_page_31.md → Pages/cains_jawbone_page_7.md`: re-read says no continuity; the best replacement lead `p31 → p21` now has a phrase-level continuation plus a plausible motion/route cue (speeding/warnings → “turned sharply” into the Henry/body scene). **Status:** placeholder. **Next:** run a minimal block-level restitch test for `p31 → p21` (and re-evaluate `p6 → p21` competition) or find a different successor for Page 31 that keeps `p13 → p31` intact.
...
- **Join (confidence: `MAYBE`):** Pages/cains_jawbone_page_31.md → Pages/cains_jawbone_page_21.md
  ...
  - **Falsifier:** re-read shows incompatible setting/voice (or the clause repetition is shown to be a detachable echo), and/or a competing predecessor for Page 21 (currently `Pages/cains_jawbone_page_6.md → Pages/cains_jawbone_page_21.md` via digitalis/foxglove) survives closer re-read and makes this join unnecessary.
