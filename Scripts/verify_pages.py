...
def md_body_before_notes(md_text: str) -> str:
    # Preserve exact original character offsets for evidence/concordance consumers.
    # Integrity comparison applies its documented normalization separately.
    match = NOTES_RE.search(md_text)
    if not match:
        raise ValueError("Missing '## Notes' section")
    if len(NOTES_RE.findall(md_text)) != 1:
        raise ValueError("Expected exactly one '## Notes' heading")
...
def main() -> int:
    ...
    expected_names = {f"cains_jawbone_page_{i}.md" for i in range(1, 101)}
    actual_names = {p.name for p in args.pages_dir.glob('cains_jawbone_page_*.md')}
    if actual_names != expected_names:
        print(f"Page inventory mismatch; missing: {sorted(expected_names - actual_names)}; "
              f"extra: {sorted(actual_names - expected_names)}", file=sys.stderr)
        return 1
    if any(p.is_symlink() for p in args.pages_dir.glob('cains_jawbone_page_*.md')):
        print('Page symlinks are not permitted.', file=sys.stderr)
        return 1
